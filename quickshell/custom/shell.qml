...
ShellRoot {
    ...
    readonly property color menuBg: Qt.alpha("#000000", 0.55)
    readonly property color hoverBg: Qt.alpha("#000000", 0.35)
    readonly property color activeBg: Qt.alpha("#000000", 0.45)
    ...
    Variants {
        ...
        PanelWindow {
            ...
            function activeMenuItem() {
                if (root.openMenu === "apps") return appMenu;
                if (root.openMenu === "wifi") return wifiMenu;
                if (root.openMenu === "bluetooth") return bluetoothMenu;
                if (root.openMenu === "power") return powerMenu;
                return null;
            }
            ...
            function pointInItem(item, x, y) {
                if (!item || !item.visible || item.width <= 0 || item.height <= 0)
                    return false;
                var p = item.mapFromItem(window.contentItem, x, y);
                return p.x >= 0 && p.x <= item.width && p.y >= 0 && p.y <= item.height;
            }

            function pointerInActiveArea(x, y) {
                var menu = activeMenuItem();
                if (pointInItem(menu, x, y))
                    return true;

                if (root.openMenu === "apps")
                    return pointInItem(appsButton, x, y);
                if (root.openMenu === "wifi")
                    return pointInItem(wifiButton, x, y);
                if (root.openMenu === "bluetooth")
                    return pointInItem(bluetoothButton, x, y);
                if (root.openMenu === "power")
                    return pointInItem(powerMeter, x, y) || pointInItem(powerButton, x, y);
            ...
            function updateMenuCloseTimer() {
                ...
                if (hoverTracker.hovered && pointerInActiveArea(hoverTracker.point.position.x, hoverTracker.point.position.y)) {
                    menuCloseTimer.stop();
                    return;
                }
            ...
            HoverHandler {
                id: hoverTracker
                onPointChanged: window.updateMenuCloseTimer()
                onHoveredChanged: window.updateMenuCloseTimer()
            }
            ...
            Item {
                ...
                readonly property int menuPad: 6
                readonly property color pillColor: Qt.alpha("#000000", 0.55)
                readonly property color pillBorder: Qt.alpha(root.border, 0.65)
                readonly property real maxSideWidth: Math.max(0, (width - centerPill.width - (pillGap * 2)) / 2)
                ...
                Rectangle {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    height: parent.height
                    readonly property real baseWidth: leftContent.implicitWidth + (bar.pillPadX * 2)
                    width: root.openMenu === "apps" ? Math.max(baseWidth, 280) : baseWidth
                    radius: height / 2
                    color: bar.pillColor
                    border.width: 1
                    ...
                    Row {
                        id: leftContent
                        anchors.left: parent.left
                        anchors.leftMargin: bar.pillPadX
                        anchors.verticalCenter: parent.verticalCenter
                        /* anchors.top: parent.top
                        anchors.topMargin: 0
                        height: bar.height
                    ...
                    Item {
                        id: appsMenuBody
                        visible: leftPill.open || opacity > 0.01
                        opacity: leftPill.open ? 1 : 0
                        anchors.left: parent.left
                        anchors.right: parent.right
                        anchors.top: parent.top
                        anchors.topMargin: bar.height
                        height: leftPill.open ? (appContent.implicitHeight + (bar.menuPad * 2)) : 0
                        clip: true
                ...
                Rectangle {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    height: parent.height
                    width: timeText.implicitWidth + (bar.pillPadX * 2)
                    radius: height / 2
                    color: bar.pillColor
                    border.width: 1
                    border.color: bar.pillBorder
                ...
                Rectangle {
                    ...
                    anchors.verticalCenter: parent.verticalCenter
                    height: parent.height
                    readonly property real baseWidth: rightContent.implicitWidth + (bar.pillPadX * 2)
                    readonly property real expandedWidth: (root.openMenu === "wifi" ? 240
                        : root.openMenu === "bluetooth" ? 220
                        : root.openMenu === "power" ? 180
                        : 0)
                    width: expandedWidth > 0 ? Math.max(baseWidth, expandedWidth) : baseWidth
                    radius: height / 2
                    color: bar.pillColor
                    border.width: 1
                    border.color: bar.pillBorder
                    ...
                    Row {
                        id: rightContent
                        anchors.right: parent.right
                        anchors.rightMargin: bar.pillPadX
                        anchors.verticalCenter: parent.verticalCenter
                        spacing: 8

                        BarButton {
                            id: wifiButton
                            readonly property var activeAp: Nmcli.active
                            icon: Nmcli.wifiEnabled ? "\ue63e" : "\ue648"
                            label: Nmcli.wifiEnabled
                                ? (activeAp ? ("Wi-Fi: " + activeAp.ssid) : "Wi-Fi")
                                : "Wi-Fi off"
                    ...
                    Item {
                        id: rightMenuBody
                        visible: rightPill.open || opacity > 0.01
                        opacity: rightPill.open ? 1 : 0
                        anchors.left: parent.left
                        anchors.right: parent.right
                        anchors.top: parent.top
                        anchors.topMargin: bar.height
                        height: rightPill.open ? (rightPill.selectedMenuHeight + (bar.menuPad * 2)) : 0
                        clip: true
            ...
            MenuPanel {
                ...
                Column {
                    ...
                    MenuRow {
                        width: parent.width
                        label: Nmcli.wifiEnabled ? "Turn Wi-Fi off" : "Turn Wi-Fi on"
                        sublabel: Nmcli.active ? Nmcli.active.ssid : "NetworkManager"
                        selected: false
                    ...
                    MenuRow {
                        width: parent.width
                        label: Nmcli.scanning ? "Scanning…" : "Rescan networks"
                        sublabel: (Nmcli.networks ? Nmcli.networks.length : 0) + " networks"
                        selected: false
                        onClicked: if (!Nmcli.scanning && Nmcli.wifiEnabled) Nmcli.rescanWifi()
                    }
                    ...
                    readonly property var networksSorted: {
                        var nets = Nmcli.networks ? [...Nmcli.networks] : [];
                        nets.sort(function(a, b) {
                            if ((a && a.active) !== (b && b.active))
                                return (b && b.active) ? 1 : -1;
                    ...
                    Repeater {
                        model: wifiContent.networksSorted.length

                        delegate: MenuRow {
                            required property int index
                            readonly property var ap: wifiContent.networksSorted[index]

                            width: wifiContent.width
                            label: ap ? ap.ssid : ""
                            sublabel: ap
                                ? ((ap.active ? "connected • " : "") + ap.strength + "% " + (ap.isSecure ? "• secure" : ""))
                                : ""
                            selected: ap && ap.active
                            onClicked: {
                                if (!ap || !Nmcli.wifiEnabled)
                                    return;
                                if (ap.active) {
                                    Nmcli.disconnectFromNetwork();
                                    root.closeMenu();
                                } else {
                                    Nmcli.connectToNetworkWithPasswordCheck(ap.ssid, ap.isSecure, function(result) {
